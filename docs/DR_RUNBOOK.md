# sm-event-bus（事件总线）容灾恢复手册（DR Runbook）

## 1. 数据库备份与恢复

- 备份：PostgreSQL 每日全量 + WAL 持续归档，库名 sm_event_bus。
- 恢复：
  1. 确认恢复目标时间点（PITR）。
  2. 在备集群拉起 sm_event_bus，回放 WAL 至目标时间。
  3. 修改 SM_DATABASE_URL（Vault）指向新主库。
  4. `kubectl rollout restart deploy/sm-event-bus -n sm-prod`。
  5. 验证连接与核心接口。
