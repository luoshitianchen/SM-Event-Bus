# sm-event-bus（事件总线）安全基线

> 适用范围：sm-event-bus 微服务，容器端口 8012，数据库 sm_event_bus。
> 本文档对应《SM-* 企业化资产模板规范 v1.0》第 103-112 节。

## 1. 安全基线概述

sm-event-bus 是 SM 安全运维平台的事件总线组件。安全基线覆盖容器运行时、网络暴露面、密钥管理、数据保护与审计五个方面，满足等保 2.0 三级与 ISO 27001 要求。容器以非 root（UID 1000）运行，根文件系统只读，去除全部 Linux capabilities，禁止提权。
