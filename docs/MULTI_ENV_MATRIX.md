...
| 域名 | sm-event-bus.dev.sm.example.com | sm-event-bus.staging.sm.example.com | sm-event-bus.sm.example.com |
| TLS | 关闭 | sm-staging-tls | sm-tls |
| 数据库 | sm_event_bus（dev 实例） | sm_event_bus（staging 实例） | sm_event_bus（primary） |
