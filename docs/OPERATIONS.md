# sm-event-bus（事件总线）运维手册

## 1. 服务概述与依赖

sm-event-bus 为 SM 平台事件总线，监听容器端口 8012。
...
1. `helm history sm-event-bus` 查看版本。
2. `helm rollback sm-event-bus <上一版本>`。
...
- 日志排查：`kubectl logs -l app.kubernetes.io/instance=sm-event-bus` 或 Loki 按 job=sm-event-bus 检索。
